...
class PagesController extends Controller
{
    public function dashboard(){
    	...
    	return view('Dashboard.index', compact('siswa','presensi'));
    }
    ...
    public function nokartu(){
        ...
        return view('siswa.nokartu',compact('data','cek'));
    ...
    public function reader(){
        ...
        if(empty($cek)){
            ...
            // rfid temporary
            $rfid_masuk = $data->uid;
            // cek rfid apakah sudah terdaftar di tabel siswa
            $siswa = Siswa::where('uid', $rfid_masuk)->count();
            if($siswa > 0){

                // ambil nama siswa
                $data = Siswa::where('uid', $rfid_masuk)->first();
                $nama = $data->nama;
                // -----------------------------------
                date_default_timezone_set('Asia/Jakarta');
                $tanggal = date('Y-m-d');
